...
let vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
            let url = vm.tCategory.id == null ? "../tcategory/save" : "../tcategory/update";
			$.ajax({
				type: "POST",
			    url: url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.tCategory),
                success: function (r) {
                    if (r.code === 0) {
		...
		getInfo: function(id){
			$.get("../tcategory/info/"+id, function (r) {
                vm.tCategory = r.tCategory;
            });
		},
